...
void main()
{
	...
	do
	{
		...
		cout << "Для выхода нажмите esc, для того чтобы сыграть еще раз нажмите любую клавишу\n";
	} while (_getch()!=27);
...
void Move(char field[], const int n, char player)
{
	...
	do
	{
		...
		do
		{
			key = _getch();
			if (key == 27)return;
			if (key < '1' || key>'9' && key != 27)cout << "Нажимай на кнопки внимательно" << endl;
		} while (key < '1' || key>'9');

		if (field[key - 49] != 0)
		{
			busy = true;
			cout << "Клeтка уже занята" << endl;
		}
